...
b_0 <- as.numeric(args[8])
b_1 <- as.numeric(args[9])
b_2 <- as.numeric(args[10])
b_3 <- as.numeric(args[11])
pr_e_giv_c <- as.numeric(args[12])
pry_base <- as.numeric(args[13])
...
ret1 <- parallel::mclapply(1:nsims, function(sim){
  ...
  for (i in 1:nrow(temp_df)) {
    ...
    temp_df$M[i] <- rbinom(n = 1, size = 1, prob = exp(b_0+b_1*temp_df$E[i]+b_2*temp_df$Y[i]+b_3*temp_df$E[i]*temp_df$Y[i])/(1+exp(b_0+b_1*temp_df$E[i]+b_2*temp_df$Y[i]+b_3*temp_df$E[i]*temp_df$Y[i])))
  }
  M <- temp_df$M
  # no missing method
